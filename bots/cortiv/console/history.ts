/**
 * 面板 `history`(Persona自报)—— workspace/ 这个 git 仓的提交流水与逐次 diff。
 *
...
function statusCard(ctx: ConsolePanelContext, st: HistoryState): HTMLElement {
  ...
  const card = ui.sheet({
    title: '介质状态',
    en: 'workspace/.git',
    desc: 'workspace/ 是一个独立的 git 仓(与项目仓无关)。控制台的编辑立即提交(署名 operator)。',
  });
...
function commitsCard(
  ...
  const filter = ui.input({
    value: st.path,
    cls: 'mono',
    placeholder: '只看某个路径,如 viewers/ 或 CONSTITUTION.md',
  ...
  if (!st.commits.length) {
    card.body.appendChild(ui.placeholder(
      st.status.repo ? '这个范围里还没有提交' : 'workspace/ 还没有建仓',
    ));
    return card.el;
  }
