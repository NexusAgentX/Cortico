/**
 * 面板 `workspace`—— workspace/ 的目录树 + 编辑器。
 *
...
class WorkspaceView {
  ...
  constructor(ctx: ConsolePanelContext, tree: WorkspaceTree) {
    ...
    const card = ui.sheet({
      title: '工作区',
      en: 'workspace/',
      desc: '这里的每一次保存都会立即提交进 workspace 的 git 仓(署名 operator),'
  ...
  private renderTree(): void {
    ...
    if (!nodes.length) {
      this.treePane.appendChild(ui.placeholder(this.filter ? '没有匹配的档案' : 'workspace/ 是空的'));
      return;
    }
  ...
  private async canLeaveCurrent(): Promise<boolean> {
    ...
    return this.ctx.ui.confirm({
      title: '放弃未保存的改动?',
      body: `workspace/${this.cur?.path} 改过还没保存。打开别的档案会丢掉这些改动。`,
      danger: true,
    });
  }
  ...
  private async openFile(node: WorkspaceNode | { path: string }, force = false): Promise<void> {
    ...
    this.pathLabel.textContent = `正在展开 workspace/${path}…`;
    try {
      ...
      this.pathLabel.textContent = `workspace/${file.path}`;
      this.pathLabel.title = `workspace/${file.path}`;
  ...
  private openNewDialog(): void {
    ...
    const dirOf = this.cur?.path.includes('/')
      ...
      : 'viewers';
    const dir = ui.input({ value: dirOf, placeholder: 'viewers' });
    ...
    const sync = (): void => { path.textContent = `workspace/${fullPath()}`; };
  ...
  private openRenameDialog(): void {
    ...
    const form = ui.h('div', 'ct-form');
    form.append(
      dimLine(this.ctx, `当前:workspace/${from}`),
      ui.field('新路径(相对 workspace/)', to),
  ...
  private async showHistory(): Promise<void> {
    ...
    back.append(
      ...
      ui.h('span', 'grow'),
      ui.h('span', 'ct-dim', `workspace/${path}`),
    );
      ...
      void Promise.all([
        ...
        ([d, f]) => {
          ...
          bar.append(
            ui.button('查看全文', {
              size: 'sm',
              onClick: () => { ui.drawer(`workspace/${path} @ ${c.hash}`, f.content); },
            }),
            ui.button('恢复为当前草稿', {
              size: 'sm',
              onClick: () => {
