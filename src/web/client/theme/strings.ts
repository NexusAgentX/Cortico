import { pick } from '../core/language.ts';

const zh = {
  modeLight: '☀ 浅色',
  modeDark: '◐ 黑夜',
  modeSystem: '◒ 跟随系统',
  appearanceLight: '浅色',
  appearanceDark: '黑夜',
  groupPaper: '纸面层级',
  groupInk: '文字与边界',
  groupState: '交互与状态',
  groupTimeline: '终端时间线',
  groupChart: '图表系列',
  token: {
    paper: '底纸',
    'paper-2': '侧栏纸',
    sheet: '主卡纸',
    'sheet-2': '次级卡纸',
    'sheet-3': '压低底色',
    ink: '主墨色',
    'ink-soft': '次级文字',
    'ink-dim': '弱化文字',
    line: '细边界',
    'line-2': '常规边界',
    'line-strong': '强调边界',
    accent: '主强调',
    'accent-2': '次强调',
    'on-accent': '强调色上文字',
    ok: '成功 / 在线',
    warn: '提醒 / 系统',
    danger: '危险 / 失败',
    'ink-blue': 'Agent 标记',
    violet: '思考标记',
    'agent-surface': 'Agent 卡片底色',
    'world-bg': '世界事件底色',
    'world-ink': '世界事件文字',
    'bubble-bg': '发送气泡底色',
    'bubble-ink': '发送气泡文字',
    'tool-result': '工具结果文字',
    'chart-hit': '缓存命中',
    'chart-miss': '未命中输入',
    'chart-output': '模型输出',
    'chart-1': '系列 1',
    'chart-2': '系列 2',
    'chart-3': '系列 3',
    'chart-4': '系列 4',
    'chart-5': '系列 5',
    'chart-6': '系列 6',
    'chart-7': '系列 7',
    'chart-8': '系列 8',
  },
  schemeArchive: '暖陶米白',
  schemeArchiveNote: '米白空间、陶土强调与克制的交互边界',
  schemeMineral: '矿物蓝灰',
  schemeMineralNote: '保留纸感，加入蓝灰冷印层次',
  schemeSepia: '褐墨卷宗',
  schemeSepiaNote: '旧档纸、褐墨与温暖的馆藏灯光',
  customScheme: '自定义方案',
  customNote: '本机自定义配色',
  copyOf: (name: string) => `${name} 副本`,
};

const en: typeof zh = {
  modeLight: '☀ Light',
  modeDark: '◐ Dark',
  modeSystem: '◒ System',
  appearanceLight: 'Light',
  appearanceDark: 'Dark',
  groupPaper: 'Paper layers',
  groupInk: 'Text & borders',
  groupState: 'Interaction & state',
  groupTimeline: 'Terminal timeline',
  groupChart: 'Chart series',
  token: {
    paper: 'Base paper',
    'paper-2': 'Sidebar paper',
    sheet: 'Main card',
    'sheet-2': 'Secondary card',
    'sheet-3': 'Recessed ground',
    ink: 'Primary ink',
    'ink-soft': 'Secondary text',
    'ink-dim': 'Muted text',
    line: 'Hairline border',
    'line-2': 'Regular border',
    'line-strong': 'Strong border',
    accent: 'Primary accent',
    'accent-2': 'Secondary accent',
    'on-accent': 'Text on accent',
    ok: 'Success / online',
    warn: 'Notice / system',
    danger: 'Danger / failure',
    'ink-blue': 'Agent marker',
    violet: 'Reasoning marker',
    'agent-surface': 'Agent card ground',
    'world-bg': 'World event ground',
    'world-ink': 'World event text',
    'bubble-bg': 'Sent bubble ground',
    'bubble-ink': 'Sent bubble text',
    'tool-result': 'Tool result text',
    'chart-hit': 'Cache hit',
    'chart-miss': 'Uncached input',
    'chart-output': 'Model output',
    'chart-1': 'Series 1',
    'chart-2': 'Series 2',
    'chart-3': 'Series 3',
    'chart-4': 'Series 4',
    'chart-5': 'Series 5',
    'chart-6': 'Series 6',
    'chart-7': 'Series 7',
    'chart-8': 'Series 8',
  },
  schemeArchive: 'Warm Clay',
  schemeArchiveNote: 'Off-white space, terracotta accents and restrained borders',
  schemeMineral: 'Mineral Slate',
  schemeMineralNote: 'Keeps the paper feel, adds cool blue-grey layers',
  schemeSepia: 'Sepia Archive',
  schemeSepiaNote: 'Aged paper, sepia ink and warm archive light',
  customScheme: 'Custom scheme',
  customNote: 'Custom palette on this machine',
  copyOf: (name: string) => `${name} copy`,
};

export const S = pick({ zh, en });
